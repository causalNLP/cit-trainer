...
class Trainer(nn.Module):
    def __init__(self, dataset, lambda_ci=0.05, lambda_R=0.3, gt_rate = 0.5, output_dir='./output/'):
    ...
    def compute_batch(self, batch_datapoint, train=True):
        ...
        # make sure the env_gt is require_grad
        env_gt.requires_grad = True
        ...
        if train:
            y = self.y_model(torch.cat([x1, x2, env_toTrain], dim=1))
            #print("qwqqqqq")
        else:
    ...
    def compute_loss(self, batch_datapoint, optimizer_dict = None):
        ...
        if optimizer_dict is not None:
            # First update the model without ground truth and y, using loss and CIT_loss
            loss = loss + CIT_loss
            ...
            # Then update the model with ground truth, using R_loss
...
def do_exp():
    ...
    parser.add_argument('--exp_name', type=str, default='simulated_IRM_toyModel_v6_newData_v2')
